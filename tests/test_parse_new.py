...
input: Dict[str, Any] = dict(
    schema_record_A={
        "name": "A",
        "type": "record",
        "fields": [{"name": "foo", "type": "string"}, {"name": "link", "type": "A"}],
    },
    schema_record_B={
        "name": "B",
        "namespace": "ns_B",
...
def test_reassemble():
    ...
    del referenced_schema_names

    reassembled_schema, missing_reassembly = reassemble_schema(
        decomposed_schema, sub_schemas, error_on_unknown=False
    )
    assert to_json_no_whitespace(reassembled_schema) == to_json_no_whitespace(
        output["schema_record_C"]
    )
    assert missing_reassembly == missing_schema_names
